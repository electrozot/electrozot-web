...
// Determine lock type and details
$is_locked = false;
$lock_type = '';
$lock_reason = '';
$locked_until = '';
...
if ($tech_status) {
    ...
    elseif ($tech_status->t_status === 'Locked') {
        ...
        if ($tech_status->t_blocked_until && strtotime($tech_status->t_blocked_until) <= time()) {
            // Auto-unlock and redirect to dashboard
...
// Extract amount from commission lock reason
$amount = '';
$date = '';
...
<body>
    ...
    <!-- Auto-check for unlock every 5 seconds -->
    <script>
        ...
        fetch('api-check-lock-status.php')
            ...
            .then(data => {
                if (data.success && !data.is_locked) {
                    // Account unlocked - redirect to dashboard
    ...
    // Check every 5 seconds for unlock
    setInterval(checkUnlockStatus, 5000);
    </script>
</body>
</html>
